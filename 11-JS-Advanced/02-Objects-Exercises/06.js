function solve(arr){
    ...
    for(let i = 0; i < orderedSystemKeys.length; i++){
        ...
        let orderedComponentKeys = Object.keys(systems[orderedSystemKeys[i]])
            .sort((a, b) => Object.keys(systems[orderedSystemKeys[i]][b]).length - Object.keys(systems[orderedSystemKeys[i]][a]).length);

        for(let j = 0; j < orderedComponentKeys.length; j++){
            let currentComponent = orderedComponentKeys[j];
            console.log(`|||${currentComponent}`);
            for(let k = 0; k < systems[currentSystem][currentComponent].length; k++){
