function solve(kingdomsArr, fightsArr) {
  ...
  let data = kingdomsArr.forEach(line => {
  ...
  for (let i = 0; i < fightsArr.length; i++) {
    let [firstKingdom, firstGeneral, secondKingdom, secondGeneral] = fightsArr[
      i
    ];
  ...
  for (let key in kingdoms) {
    ...
    let wins = values.map(x => x.wins).reduce((a, b) => a + b, 0);
    let losses = values.map(x => x.losses).reduce((a, b) => a + b, 0);
  ...
  orderedKingdoms = orderedKingdoms.sort(
    (a, b) =>
  ...
  let winnerKingdomEntries = Object.entries(kingdoms[winnerKingdomKey]).sort(
    (a, b) => b[1].army - a[1].army
  );
