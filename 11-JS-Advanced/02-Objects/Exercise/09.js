...
function solve(arr) {
    ...
    if (
      ...
      for (let j = 0; j < firstGladiatorTechniques.length; j++) {
        ...
        if (secondGladiatorTechniques.includes(currentTechnique)) {
          let firstGladiatorSkill =
            gladiators[firstGladiator][currentTechnique];
          let secondGladiatorSkill =
            gladiators[secondGladiator][currentTechnique];

          if (firstGladiatorSkill === secondGladiatorSkill) {
            continue;
          }

          firstGladiatorSkill > secondGladiatorSkill
  ...
  let orderedGladiatorKeys = Object.entries(gladiatorsTotalSkill)
    .sort((a, b) => b[1] - a[1])
    .map(x => x[0]);
  ...
  for (let gladiator of orderedGladiatorKeys) {
    ...
    let techniqueKeys = Object.keys(gladiators[gladiator]).sort((a, b) =>
      b.localeCompare(a)
    );
