function solve() {
  ...
  document
    .querySelectorAll("#container button")[1]
    .addEventListener("click", () => {
      let checkedMarks = [...document.querySelectorAll(".table input")].filter(
        m => m.checked === true
      );
      ...
      let totalPrice = [...checkedMarks].reduce((a, b) => {
        return (
          a + Number(b.parentNode.parentNode.childNodes[2].firstChild.innerHTML)
        );
      }, 0);

      let avgDecFactor =
        [...checkedMarks].reduce((a, b) => {
          return (
            a + Number(b.parentNode.parentNode.childNodes[3].firstChild.innerHTML)
          );
