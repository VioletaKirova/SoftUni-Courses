function solve(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    let drinkPrice =
      order[1] === "coffee"
        ? drinkPricesMap[order[2]]
        : drinkPricesMap[order[1]];

    let milk =
      order[order.length - 2] === "milk" ? (drinkPrice * 0.1).toFixed(1) : 0;
    ...
    if (change < 0) {
      console.log(
        `Not enough money for ${order[1]}. Need $${(change * -1).toFixed(
          2
        )} more.`
      );
    } else {
      console.log(
        `You ordered ${order[1]}. Price: $${drinkPrice.toFixed(
          2
