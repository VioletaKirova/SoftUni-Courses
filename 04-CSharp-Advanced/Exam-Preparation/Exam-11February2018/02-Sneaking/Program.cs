...
namespace _02_Sneaking
{
    class Program
    {
        ...
        private static void SamStepsOnEnemy(int targetCol)
        {
            if (jaggedArray[samRow][targetCol] == 'b' || jaggedArray[samRow][targetCol] == 'd')
            {
        ...
        private static void CheckUpDownRow(int targetRow)
        {
            ...
            else if (targetRowStr.Contains('N'))
            {
        ...
        {
            string nikoladzeRow = string.Join("", jaggedArray[targetRow]);

            int indexOfNikoladze = nikoladzeRow.IndexOf('N');

            jaggedArray[targetRow][indexOfNikoladze] = 'X';
        ...
        private static void FillJaggedArray(int rows, List<int> enemiesRows)
        {
            for (int row = 0; row < rows; row++)
            {
                ...
                jaggedArray[row] = new char[currentRow.Length];
